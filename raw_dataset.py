...
class TypedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        #print("dataloader rank", _get_rank(), "idx", idx)
...
class TypedStreamingCLMDataCollator:
    ...
    def concatenate_if_needed(self, example) -> str: #text: str, is_conversation: bool) -> str:
        ...
        if attempts > max_attempts:
            print("TOO MANY ATTEMPTS toklen", len(tokens['input_ids']))
    ...
    def __call__(self, examples: List[Dict[str, Union[str, List[str]]]]) -> Dict[str, torch.Tensor]:
        ...
        for example in examples:
            processed_text = self.concatenate_if_needed(example) #if self.packing else text
