...
def add_raw(raw):
    global builder, cnt
    out = tokenizer.encode(raw, add_special_tokens=False)
    # NOTE - interestingly, the qwen tokenizer is inconsistent for hebrew, such that the result is visually the same but the unicode is not!
    #   the difference is in the diacritical marks
    # decoded = tokenizer.decode(out, skip_special_tokens=True) 
    # if decoded != raw:
    #     def find_mismatch_indices(s1, s2):
    #         return [i for i in range(min(len(s1), len(s2))) if s1[i] != s2[i]] or None
    #     mismatch_indices = find_mismatch_indices(raw, decoded)
    #     if mismatch_indices is not None:
    #         first_mismatch = mismatch_indices[0]
    #     else:
    #         if len(raw) != len(decoded):
    #             first_mismatch = min(len(raw), len(decoded))
    #             print("Mismatched lengths: raw:", len(raw), " decoded:", len(decoded))
    #         else:
    #             print("UGH Mismatch wasn't found?!?!")
    #     print(raw[first_mismatch-50:first_mismatch], "\n[MISMATCH]", raw[first_mismatch:first_mismatch+50])
    #     print(decoded[first_mismatch-50:first_mismatch], "\n[MISMATCH]", decoded[first_mismatch:first_mismatch+50])
    #     print("ERROR" * 100)
    #     exit(0)
    out.append(tokenizer.eos_token_id)  # [0] = end_of_doc for rwkv tokenizer
    builder.add_item(np.array(out, dtype=np.uint32))
...
if ARG1 != 'check':
    ...
    for i in range(N_EPOCH):
        ...
        # FIXME - we should sort large to small or small to large instead
...
print(f"### Verifying result for {OUT_NAME}.bin/idx...")
