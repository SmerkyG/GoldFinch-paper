...
__global__ void forward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, float* s0_, bf* y_, float* s_, float* sa_, float* sT_) {
    constexpr int C = _C_;
    int bb = blockIdx.z, hh = blockIdx.y, basei = blockIdx.x*K, i = threadIdx.x, rowi_ = basei+i%K, basej = i/K*K;

    float state[K];
    for (int j = 0; j < K; j++) {
        state[j] = s0_[bb*H*C*C + hh*C*C + rowi_*C + basej+j];
    }
...
#pragma unroll
    ...
    for (int j = 0; j < K; j++) {
        sT_[bb*H*C*C + hh*C*C + rowi_*C + basej+j] = state[j];
    }
}

__global__ void backward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, F_ dy_, float * __restrict__ s_, float * __restrict__ sa_, float* dsT_, float* dw_, float* dq_, float* dk_, bf* dv_, float* da_, float* db_, float* ds0_) {
    constexpr int C = _C_;
    int bb = blockIdx.z, hh = blockIdx.y, basei = blockIdx.x*K, i = threadIdx.x, rowi = i%K, basej = i/K*K;

    float stateT[K], dstate[K], dstateT[K];
    for (int j = 0; j < K; j++) {
        dstate[j] = dsT_[(bb*H+hh)*C*C + (basei+rowi)*C + basej+j];
        dstateT[j] = dsT_[(bb*H+hh)*C*C + (basei+j)*C + i];
    }
...
#pragma unroll
    ...
    for (int j = 0; j < K; j++) {
        ds0_[(bb*H+hh)*C*C + (basei+rowi)*C + basej+j] = dstate[j];
    }
}
...
void cuda_backward(int B, int T, int H, bf*w, bf*q, bf*k, bf*v, bf*a, bf*b, bf*dy, float*s, float*sa, float*dsT, float*dw, float*dq, float*dk, bf*dv, float*da, float*db, float*ds0) {
