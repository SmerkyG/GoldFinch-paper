...
class RWKV7Attention(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        self.dim_posemb = self.num_heads * int(config.partial_posemb * self.head_dim)

        self.w0 = nn.Parameter(torch.empty(1,1,self.dim_posemb))
        self.w1 = nn.Parameter(torch.empty(C, lora_rank_decay))
        self.w2 = nn.Parameter(torch.empty(lora_rank_decay, self.dim_posemb))
    ...
    def forward(
        ...
        if self.dim_posemb != self.head_dim:
            log_neglog_w = F.pad(log_neglog_w.view(B, T, self.num_heads, self.dim_posemb // self.num_heads), [0, self.head_dim - (self.dim_posemb // self.num_heads)], value=float('-inf')).view(B, T, -1)
